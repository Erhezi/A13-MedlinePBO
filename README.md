...
- **Medline PBO** — entry point `run_medline_pbo.py`, config `reports/medline_pbo/config.yaml`, launcher `run_medline_pbo.bat` (scheduled Tue/Thu).
- **Medline Allocation** — entry point `run_medline_allocation.py`, config `reports/medline_allocation/config.yaml`, launcher `run_medline_allocation.bat` (scheduled weekly).
...
When `run_medline_pbo.py` runs, it:
...
1. `run_medline_pbo.py` and `run_medline_allocation.py` (report entry points)
...
This batch file always targets the project's own `.venv\Scripts\python.exe`, `run_medline_pbo.py`, and its `config.yaml` by absolute path derived from the batch file location, so it does not depend on the caller's working directory.
...
This is the safest option because the batch file builds absolute paths to `.venv\Scripts\python.exe`, `run_medline_pbo.py`, and its `config.yaml` from its own folder.
...
```text
"your_path_to_this_folder\A13-MedlinePBO\run_medline_pbo.py" --config "your_path_to_this_folder\A13-MedlinePBO\reports\medline_pbo\config.yaml"
```
...
4. If Steps 1, 2, and 3 all show a value, but `python run_medline_pbo.py` still fails, the stored passphrase likely does not match the encrypted values in `.env`.
